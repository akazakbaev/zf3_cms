...
class Gd extends Image {
    ...
    public function __construct($options = array()) {
        ...
        if (!function_exists('gd_info')) {
            throw new ImageException('GD library is not installed');
        }
    ...
    public function create($width, $height) {
        ...
        if (!$resource) {
            throw new ImageException("Unable to create image");
        }
     ...
     * @throws ImageException If unable to open
     */
    public function open($file) {
        ...
        if (!$info) {
            throw new ImageException(sprintf("File \"%s\" is not an image or does not exist", $file));
        }
        ...
        if (!function_exists($function)) {
            throw new ImageException(sprintf('Image type "%s" is not supported', $type));
        }

        // Open
        $this->_resource = $function($file);
        if (!$this->_checkOpenImage(false)) {
            throw new ImageException("Unable to open image");
        }
     ...
     * @return Azbe\Image\Adapter\Gd
     * @throws ImageException If unable to write
     */
    public function write($file = null) {
        // If no file specified, write to existing file
        if (null === $file) {
            if (null === $this->_file) {
                throw new ImageException("No file to write specified.");
            }
            $file = $this->_file;
        }
        ...
        if (!function_exists($function)) {
            throw new ImageException(sprintf('Image type "%s" is not supported', $outputFormat));
        }
     ...
     * @return mixed
     * @throws ImageException If unable to output
     */
...
//    }
        ...
        if (!$result) {
            ...
            throw new ImageException("Unable to output image");
        }
     ...
     * @return Azbe\Image\Adapter\Gd
     * @throws ImageException If unable to resize
     */
     ...
     * @param integer $h
     * @return Azbe\Image\Adapter\Gd
     * @throws ImageException If unable to crop
     */
    ...
    public function rotate($angle) {
        ...
        if (!$result) {
            imagedestroy($result);
            throw new ImageException('Unable to rotate image');
        }
    ...
    protected static function _isSafeToOpen($width, $height, $bpp = 4) {
        ...
        if ($required > $available) {
            throw new ImageException(sprintf('Insufficient memory to open ' .
     ...
     * @param boolean $throw
     * @return boolean
     * @throws ImageException If $throw is true and not supported
     */
