...
class LanguageTreeRouteStack extends TranslatorAwareTreeRouteStack {
	...
	function setLanguageOptions(LanguageOptions $languageOptions)
    {
	...
	function setAuthManager(AuthManager $authenticationService)
    {
		$this->authManager = $authenticationService;
	}
	...
	public function match(RequestInterface $request, $pathOffset = null, array $options = array())
    {
		...
		elseif(!empty($this->getAuthManager()) && $this->getAuthManager()->getIdentity())
        {
			// try to get user language if no language was provided by url
			$user = $this->getAuthManager()->getViewer();

			if($user instanceof \User\Entity\UserUsers)
			{
