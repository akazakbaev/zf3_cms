...
use Storage\Exception\VfsAdapterException;

class Ssh extends AbstractRemote
{
  protected $_port = 22;
  ...
  public function __construct(array $config = null)
  {
    if( !extension_loaded('ssh2') ) {
      throw new VfsAdapterException('The ssh2 extension is not installed, unable to initialize SSH-VFS');
    }
  ...
  public function getSftpResource()
  {
    if( null === $this->_sftpResource ) {
      ...
      if( null === $this->_sftpResource ) {
        throw new VfsAdapterException('Unable to get sftp resource');
      }
    }
  ...
  public function connect()
  {
    ...
    if( !$resource ) {
      throw new VfsAdapterException(sprintf('Unable to connect to "%s"', $this->getHost()));
    }
  ...
  public function disconnect()
  {
    if( null !== $this->_resource ) {
      ...
      if( !$return ) {
        throw new VfsAdapterException('Disconnect failed.');
      }
      $this->_resource = null;
    }
  ...
  public function login()
  {
    ...
    // Failure
    if( !$return ) {
      throw new VfsAdapterException('Login failed.');
    }
  ...
  public function command($command)
  {
    ...
    if( !$stream ) {
      throw new VfsAdapterException(sprintf('Unable to execute command "%s"', $command));
    }
  ...
  {
    // @todo more fun stuff
    throw new VfsAdapterException('Disconnected from server');
  }
    ...
    if( null === $this->_systemType ) {
      if( substr($this->printDirectory(), 1, 2) == ':\\' ) {
        ...
        if( !$systype ) {
          ...
          throw new VfsAdapterException(sprintf('Unknown remote system type'));
          //return self::SYS_LIN;
        }
  ...
  public function copy($sourcePath, $destPath)
  {
    ...
    $tmpFile = tempnam('/tmp', 'store_vfs') . basename($sourcePath);
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to copy "%s" to "%s"', $sourcePath, $destPath));
    }
  ...
  public function get($local, $path)
  {
    ...
    // Error
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to get "%s" to "%s"', $path, $local));
    }
  ...
  public function getContents($path)
  {
    ...
    if( !$contents ) {
      throw new VfsAdapterException(sprintf('Unable to get contents of "%s"', $path));
    }
  ...
  public function mode($path, $mode, $recursive = false)
  {
    ...
    if( !$this->exists($path) ) {
      throw new VfsAdapterException(sprintf('Unable to change mode on "%s"; it does not exist', $path));
    }

    $return = $this->command(sprintf('chmod ' . ($recursive ? '-R ' : ''). ' %o %s', self::processMode($mode), escapeshellarg($path)));
    if( '' != $return ) {
      throw new VfsAdapterException(sprintf('Unable to change mode on "%s" - %s', $path, $return));
    }
  ...
  public function move($oldPath, $newPath)
  {
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to rename "%s" to "%s"', $oldPath, $newPath));
    }
  ...
  public function put($path, $local)
  {
    ...
    // Error
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to put "%s" to "%s"', $local, $path));
    }
  ...
  public function putContents($path, $data)
  {
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to put contents to "%s"', $path));
    }
  ...
  public function unlink($path)
  {
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to unlink "%s"', $path));
    }
  ...
  public function changeDirectory($directory)
  {
    ...
    if( !$this->isDirectory($directory) ) {
      throw new VfsAdapterException(sprintf('Unable to change directory to "%s", target is not a directory', $directory));
    }
    ...
    if( $directory != $curDir ) {
      throw new VfsAdapterException(sprintf('Unable to change directory to "%s", new directory did not match (%s)', $directory, $curDir));
    }


    if( !$return && false ) {
      throw new VfsAdapterException(sprintf('Unable to change directory to "%s"', $directory));
    }
    */
  ...
  public function makeDirectory($directory, $recursive = false)
  {
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to make directory "%s"', $directory));
    }
  ...
  public function printDirectory()
  {
    if( null === $this->_path ) {
      ...
      if( !$pwd ) {
        throw new VfsAdapterException('Unable to get working directory');
      }
      $this->_path = $pwd;
    }
    return $this->_path;
  ...
  public function removeDirectory($directory, $recursive = false)
  {
    ...
    if( !$return ) {
      throw new VfsAdapterException(sprintf('Unable to remove directory "%s"', $directory));
    }
