...
use Storage\Provider\VfsAdapterInterface;

abstract class AbstractVfsAdapter implements VfsAdapterInterface
{
  ...
  {
    throw new VfsAdapterException(sprintf('Method "%s" not supported in class "%s"', $method, get_class($this)));
  }
  ...
  static public function processMode($mode, $asOct = false)
  {
    if( is_string($mode) ) {
      ...
      if( preg_match('/^[0-1]?([0-7][0-7][0-7])$/', $mode, $m) ) {
        ...
        throw new VfsAdapterException(sprintf('Unable to process mode: %s', $mode));
      }
    ...
    switch( strtoupper(substr($systype, 0, 3)) ) {
      ...
      case 'OPE':
        if( stripos($systype, 'BSD') === false ) {
          throw new VfsAdapterException(sprintf('Unknown remote system type: %s', $systype));
        }
      case 'BSD':
        return self::SYS_BSD;
        break;
